...
for i in range(1,len(trainingData)):
	if trainingData[i][3] == "TIN":
		X.append(trainingData[i][1])
		Y.append(trainingData[i][4])
...
if no == 1:
	...
	min_max = MinMaxScaler()
	X = min_max.fit_transform(X)
	# select=SelectFromModel(LogisticRegression(solver='lbfgs',multi_class = 'multinomial',random_state = 0))   
	model = svm.SVC(C=1000,gamma=0.0001)


# select.fit(X,Y) 
# x_select=select.transform(X) 
# pickle.dump(select, open("selector.pk", "wb"))
X_train, X_cross_validation , Y_train , Y_cross_validation = train_test_split(X,Y,test_size = 0.2 , random_state = 0)
